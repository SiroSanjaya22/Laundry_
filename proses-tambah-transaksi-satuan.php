...
// }else{
if(empty($_POST["No_Order"]) || empty($_POST["No_Identitas"]) || empty($_POST["total_bayar"]) || empty($_POST["Total_harga_pakaian"]) || empty($_POST["tanggal"])){
	echo "<script language='javascript'>alert('Gagal di tambahkan');</script>";
	echo '<meta http-equiv="refresh" content="0; url=tambahdatatransaksisatuan.php">';
}else{
	$sql = "INSERT INTO `transaksi` (`No_Order`, `No_Identitas`, `Tgl_Terima`, `Tgl_Ambil`,  `Total_Bayar`, `Total_harga_pakaian`)
			VALUES ('$No_Order', '$No_Identitas', '$Tgl_Terima', NULL,   '$total_bayar', '$Total_harga_pakaian')";
