...
<div class="container">
  ...
  <hr>
        <div class="row">
          <div class="col-md-4">
            <form name="form" action="proses-tambah-transaksi-satuan.php" method="POST" >
            <?php
            ...
            <?php }} ?>              
                ...
                <div class="form-group">
                  <label>Total Harga Pakaian</label>
                  <input type="text" id="Total_harga_Pakaian" class="form-control" name="Total_harga_pakaian" placeholder="Total harga" value="0">
                </div>
                <div class="form-group">
              ...
              <table id="table" class="table table-striped table-bordered" >
                ...
                <tbody>
                  <?php
                    ...
                    $no_o = $no + 1;
                    $i = 0 + 1;
                    $sql = mysqli_query($conn, "SELECT pakaian.Jenis_Pakaian, pakaian.harga_pakaian, detail_transaksi.No_Order, detail_transaksi.Id_Pakaian, detail_transaksi.Jumlah_pakaian, detail_transaksi.Total_harga FROM detail_transaksi join pakaian on detail_transaksi.Id_Pakaian = Pakaian.Id_Pakaian Where No_Order = $no_o");
                    while ($hasil = mysqli_fetch_array($sql)) {
                 ?>
              <tr>
...
<!-- Modal Tambah Data -->
		<div class="modal fade" id="ModalTambah" role="dialog">
			<div class="modal-dialog modal-sm">
              ...
              <div class="form-group">
                  <label>Masukan Harga Pakaian</label>
                  <select class="form-control" name="harga_pakaian">
                    <?php
                    $sql = mysqli_query($conn, "SELECT * FROM pakaian");
                    while ($hasil = mysqli_fetch_array($sql)){
                      ?>
                    <option value="<?php echo $hasil['harga_pakaian']; ?>"><?php echo $hasil['Jenis_Pakaian']; ?> (<?php echo $hasil['harga_pakaian']; ?>)</option>
                    <?php
                        }
                    ?>
                  </select>
...
function tambah()
    {
    a=eval(form.Total_harga_pakaian.value)


    // b=eval(form.b.value)
    // c=a+b
  
    form.total_bayar.value=a
    }
